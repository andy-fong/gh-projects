...
pub fn KpiStrip(totals: StatsTotals) -> Element {
    ...
    rsx! {
        div { class: "ts-kpis",
            ...
            KpiCard {
                label: "Merged, nobody approved".to_string(),
                value: t.merged_without_approval.to_string(),
                sub: format!("of {} merged in this window", t.prs_merged),
                tone: if t.merged_without_approval > 0 { "warn".to_string() } else { "good".to_string() },
            }
            KpiCard {
